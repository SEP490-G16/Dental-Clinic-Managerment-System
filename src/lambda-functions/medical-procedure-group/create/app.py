...
def lambda_handler(event, context):
    ...
    try:
        ...
        query = """INSERT INTO `medical_procedure_group` (`name`, `description`)
                VALUES (%s, %s);"""

        cursor.execute(query, (data.get('name'), get_value_or_none(data, 'description')))
        
        cursor.execute("SELECT medical_procedure_group_id FROM medical_procedure_group ORDER BY medical_procedure_group_id DESC LIMIT 1;")
        row = cursor.fetchone()
        patient_id = row[0]
        conn.commit()

        return create_response(201, message='Medical procedure group created successfully', data= {'medical_procedure_group_id': patient_id})
