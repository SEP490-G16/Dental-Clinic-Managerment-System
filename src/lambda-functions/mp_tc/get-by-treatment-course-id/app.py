...
def lambda_handler(event, context):
    ...
    if ('pathParameters' not in event or
            'treatment-course-id' not in event['pathParameters'] or
            not event['pathParameters']['treatment-course-id'] or
    ...
    try:
        ...
        query = """
            SELECT * FROM `mp_tc`
            WHERE status == 1 AND treatment_course_id = %s
        """
        cursor.execute(query, (event['pathParameters']['treatment-course-id']))
