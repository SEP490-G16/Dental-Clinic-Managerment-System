...
def lambda_handler(event, context):
    ...
    if event['httpMethod'] != 'PUT' or not event.get('body') or not event.get('pathParameters') or 'treatment-course-id' not in event['pathParameters'] or 'medical-procedure-id' not in event['pathParameters']:
        return create_response(400, 'Bad Request')

    try:
        t_id = event['pathParameters']['treatment-course-id']
        m_id = event['pathParameters']['medical-procedure-id']
        ...
        query = """
            UPDATE `mp_tc` 
            SET `treatment_course_id` = %s
            WHERE treatment_course_id=%s AND medical_procedure_id=%s;
            """
        cursor.execute(query, (get_value_or_none(data, 'treatment_course_id'), 
                               t_id,
                               m_id))

        conn.commit()
        if cursor.rowcount == 0:
            response =  create_response(status_code=404, message='Treatment course not found or not change')
        else:
            response = create_response(status_code=200, message='Treatment course updated successfully') 
