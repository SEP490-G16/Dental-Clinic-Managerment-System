...
def lambda_handler(event, context):
    ...
    if event['httpMethod'] != 'DELETE' or not event.get('pathParameters') or 'patient_id' not in event['pathParameters']:
        return create_response(400, message='Material usage not found')
    try:
        ...
        cursor = conn.cursor()
        query = "UPDATE `x_ray_image` SET `status` = 0 WHERE `patient_id` = %s, `url` = %s"
        cursor.execute(
            query, (event['pathParameters']['patient_id'], event['pathParameters']['url']))
        conn.commit()
        response = create_response(
            200, message='X-ray image deactivated successfully')
