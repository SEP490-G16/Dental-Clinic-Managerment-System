...
def lambda_handler(event, context):
    ...
    try:
        ...
        query = """
            UPDATE `labo` 
            SET name=%s, description=%s, phone_number=%s, email=%s, address=%s
            WHERE labo_id=%s;
            """
        ...
        if cursor.rowcount == 0:
            return create_response(status_code=404, message='Labo not found')

        return create_response(status_code=200, message='Labo updated successfully') 
