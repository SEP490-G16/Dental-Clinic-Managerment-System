...
def lambda_handler(event, context):
    ...
    try:
        ...
        cursor = conn.cursor()
        query = """
            UPDATE `paid_material_usage` 
            SET `material_usage_id` = %s, `total_paid` = %s
            WHERE paid_material_usage_id=%s;
            """
        cursor.execute(query, (get_value_or_none(data, 'material_usage_id'), 
                               get_value_or_none(data, 'total_paid'),
                               id))

        conn.commit()
        if cursor.rowcount == 0:
            response =  create_response(status_code=404, message='Paid material usage not found or not change')
        else:
            response = create_response(status_code=200, message='Paid material usage updated successfully') 
