...
def lambda_handler(event, context):
    ...
    if event['httpMethod'] != 'DELETE' or not event.get('pathParameters') or 'id' not in event['pathParameters']:
        return create_response(400, message='Paid material usage not found')
    try:
        ...
        query = "UPDATE `paid_material_usage` SET `status` = 0 WHERE paid_material_usage_id=%s;"
        cursor.execute(query, (id))
        conn.commit()
        response = create_response(200, message='Paid material usage deactivated successfully')
