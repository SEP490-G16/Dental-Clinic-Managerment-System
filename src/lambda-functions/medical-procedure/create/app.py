...
def lambda_handler(event, context):
    ...
    try:
        ...
        query = """INSERT INTO `medical_procedure` (`medical_procedure_group_id`, `name`, `price`, `description`)
                VALUES (%s, %s, %s, %s);"""
        ...
        cursor.execute("SELECT medical_procedure_id FROM medical_procedure ORDER BY medical_procedure_id DESC LIMIT 1;")
        row = cursor.fetchone()
        patient_id = row[0]
        conn.commit()
