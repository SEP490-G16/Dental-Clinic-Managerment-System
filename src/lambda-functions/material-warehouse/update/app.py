...
def lambda_handler(event, context):
    ...
    if event['httpMethod'] != 'PUT' or not event.get('body') or not event.get('pathParameters') or 'material_id' not in event['pathParameters'] or 'inport_material_id' not in event['pathParameters']:
    ...
    try:
        ...
        query = """
            ...
            WHERE material_id=%s AND inport_material_id=%s;
            """
