...
def lambda_handler(event, context):
    ...
    if event['pathParameters']['type'] not in ['order-date', 'received-date', 'used-date']:
      return create_response(400, 'Invalid query date type')
    try:
    ...
    try:
        ...
        query = """
            ...
            LIMIT 11 OFFSET %s;
        """
        cursor.execute(query, (attribute_query, event['pathParameters']['start-date'], event['pathParameters']['end-date'], offset))
