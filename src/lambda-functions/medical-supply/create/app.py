...
def lambda_handler(event, context):
    ...
    required_fields = ['name', 'quantity', 'facility_id']
    ...
    try:
        ...
        cursor.execute(
            "SELECT medical_supply_id FROM medical_supply ORDER BY medical_supply_id DESC LIMIT 1;")
        row = cursor.fetchone()
        id = row[0]
        conn.commit()
        response = create_response(
            201, message='Medical supply created successfully', data={'medical_supply_id': id})
