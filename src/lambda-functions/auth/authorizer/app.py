...
def lambda_handler(event, context):
    ...
    admin_permission = ['POST/patient/name', ]
    if user_info is None:
        return create_policy(effect, event['methodArn'])
    
    custom_role_value = get_value_of_name(user_info['UserAttributes'], 'custom:role')
    
    if '1' in custom_role_value and contains_any(str(event['methodArn']), admin_permission):
        return create_policy('Allow', str(event['methodArn']))
    return create_policy(effect, event['methodArn'])
